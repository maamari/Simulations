...
void main()
{
	...
	if(polarLat < 90.0 && polarLat > 0.0)
		polarLat
		    += (noise(vec4(pos, 1.0 + seed), 24, 1, 0.8) * 10.0)
		       * sqrt((90.0 - polarLat));
	...
	snowProportion
	    = 1.0
	      - clamp(15000.0 * (1.0 - cos(polarLat) - abs(pos.z)) + 0.5,
	              0.0, 1.0);

	outColor = vec4(vec3(heightmap(pos)) * color, 1.0);
